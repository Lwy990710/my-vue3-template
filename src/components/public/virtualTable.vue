<template>
  <div :style="{width:'99.5%'}" class="table table-virtual">
    <el-auto-resizer>
      <template #default="{ width }">
        <el-table-v2
          ...
          :row-height="36"
          :header-height="40"
          :expand-column-key="needExpand?columns[1].key:''"
    ...
    <div v-if="needPagination" class="page">
      <el-pagination
...
const _emits = defineEmits([
  'refresh', 'editRow', 'deleteRow', 'selectionChange', 'sizeChange', 'currentChange', 'editTableHeader', 'selectionChangeRow'
])
...
const initColumns = function(){
  ...
  columns.value.forEach(item => {
    ...
    item.cellRenderer = ({ rowData, column, rowIndex }) => {
      // 格式化数据
      let forMatValue = _props.forMatData(column.dataKey, rowData[column.dataKey])
...
const initHandleColumns = () => {
  columns.value.push({
    ...
    cellRenderer: ({ rowData }) => {
      ...
      } else return _props.customizeEndHandle(rowData)
...
watch([() => _props.total, () => _props.tableData.length, () => _props.pageSize, () => _props.currentPage], () => {
  ...
  if (tableData.value.length * 36 > 800 || tableData.value.length === 0) height.value = 800
  else height.value = tableData.value.length * 36 + 40
})
...
.empty{
  ...
  .img{
    ...
    width: 20%;
    height: 300px;
    background-image: url('../../../public/assets/images/home/empty.png');
  }
  .label{
    font-weight: 700;
    font-size: 16px;
...
:deep(.el-table-v2__header-cell){
  border-top: 1px solid rgba(192, 192, 192,.5) !important;
...
:deep(.el-table-v2__header-row){
  border-right:1px solid rgba(192, 192, 192,.5) !important;
}
